...
export interface IBlog extends Document {
  title: { en: string; hi: string }; slug: string;
  content: { en: string; hi: string }; excerpt: { en: string; hi: string };
  coverImage?: string; category: string; tags: string[]; author: string;
  isPublished: boolean; publishedAt?: Date;
  seo: { title: string; description: string }; views: number;
}
...
const BlogSchema = new Schema<IBlog>({
  ...
  coverImage: { type: String }, category: { type: String, default: 'vastu' },
  tags: [{ type: String }], author: { type: String, default: 'Dr. PPS' },
  isPublished: { type: Boolean, default: false }, publishedAt: { type: Date },
