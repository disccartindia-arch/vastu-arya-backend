...
router.post('/vastu-analysis', upload.array('images', 4), async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      // Provider-outage fallback. Never leaves the UI broken — but
      // NEVER pretends to have real answers. `needsMoreInfo:true`
      // tells the FE to prompt the user to try again shortly.
      res.json({
        success: true,
        data: {
          greeting: 'Namaste 🙏',
          analysis: "I'm having trouble reaching our AI engine right now. Please try again in a moment, or book a personal consultation.",
          needsMoreInfo: true,
          clarifyingQuestions: [
            'Please retry your question in a few seconds.',
            'If the issue persists, our team can help — WhatsApp us for a fast reply.',
          ],
          disclaimer: settings.showDisclaimer ? settings.disclaimerText : undefined,
          consultationCTA: settings.showConsultationCTA ? settings.ctaText : undefined,
          confidence: 'low',
