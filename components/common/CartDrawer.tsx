...
export default function CartDrawer() {
  const { showCartDrawer, setShowCartDrawer } = useUIStore();
    ...
    <>
      ...
      <div className={`fixed top-0 right-0 h-full w-full max-w-sm bg-white shadow-2xl z-50 transition-transform duration-300 flex flex-col ${showCartDrawer ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="bg-saffron-gradient p-4 flex items-center justify-between">
          <h2 className="font-display font-bold text-white text-lg flex items-center gap-2"><ShoppingCart size={20}/>{t('cart.title')}</h2>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {items.length === 0 ? (
            <div className="text-center py-16">
              ...
              <Link href="/vastu-store" onClick={() => setShowCartDrawer(false)} className="mt-4 inline-block text-primary font-semibold text-sm hover:underline">{t('cart.continueShopping')}</Link>
            </div>
          ) : (
            items.map(item => (
              <div key={item.product._id} className="flex gap-3 p-3 bg-cream rounded-xl">
                <div className="w-16 h-16 bg-orange-100 rounded-lg flex-shrink-0 flex items-center justify-center text-2xl overflow-hidden">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-text-dark truncate">{item.product.name.en}</p>
        ...
        {items.length > 0 && (
          <div className="p-4 border-t border-orange-100 bg-white">
            <div className="flex justify-between mb-3"><span className="font-semibold text-text-mid">{t('cart.total')}:</span><span className="font-display font-bold text-xl text-primary">{formatPrice(totalPrice())}</span></div>
            <Link href="/checkout" onClick={() => setShowCartDrawer(false)} className="block w-full bg-primary hover:bg-primary-dark text-white py-3.5 rounded-xl font-bold text-center transition-all shadow-orange">{t('cart.checkout')}</Link>
          </div>
        )}
      </div>
    </>
