...
export default function HeroSection({ onBookClick, settings, heroBgTheme: propTheme }: Props) {
  ...
  const trustBadges = settings?.trustBadges?.length ? settings.trustBadges.sort((a,b)=>a.order-b.order).map(b=>b.label) : ['IVAF Awarded','45,000+ Consultations','New Delhi Recognized'];
  const stats = settings?.stats?.length ? settings.stats.sort((a,b)=>a.order-b.order) : [{ value:'45,000+', label:'Happy Clients' },{ value:'15+', label:'Years Experience' },{ value:'100+', label:'Services' },{ value:'50+', label:'Cities Served' }];
  ...
  const sectionBg = isLight ? 'linear-gradient(135deg, #FFFDF7 0%, #FFF8EE 45%, #FEF3E2 100%)' : 'linear-gradient(135deg, #0D0500 0%, #1A0A00 50%, #2D1000 100%)';
    ...
    <section className="relative min-h-screen flex items-center overflow-hidden" style={{ background: sectionBg }}>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 py-20 lg:py-32 w-full">
        <div className="max-w-3xl">
          ...
          {title2 && <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.2 }} className="font-display text-4xl sm:text-5xl lg:text-7xl font-bold leading-tight mb-6 gradient-text">{title2}</motion.h1>}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} className="flex flex-wrap gap-2 mb-8">
            {trustBadges.map((badge, i) => (<span key={i} className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full" style={{ background: 'rgba(255,107,0,0.08)', color: '#5C3D1E' }}><Award size={11} style={{ color: '#D4A017' }} /> {badge}</span>))}
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="flex flex-col sm:flex-row gap-4 mb-16">
            {cta1Link === '/book-appointment' ? (
              <button onClick={onBookClick} className="group flex items-center justify-center gap-2 bg-primary hover:bg-primary-dark text-white px-8 py-4 rounded-2xl font-bold text-lg transition-all shadow-orange-lg animate-pulse-orange">{cta1Text}<ArrowRight size={20} /></button>
            ) : (
              <Link href={cta1Link} className="group flex items-center justify-center gap-2 bg-primary hover:bg-primary-dark text-white px-8 py-4 rounded-2xl font-bold text-lg transition-all shadow-orange-lg animate-pulse-orange">{cta1Text}<ArrowRight size={20} /></Link>
            )}
            <Link href={cta2Link} className="flex items-center justify-center gap-2 px-8 py-4 rounded-2xl font-bold text-lg transition-all" style={{ background: 'rgba(255,107,0,0.08)', border: '1px solid rgba(255,107,0,0.25)', color: isLight ? '#1A0A00' : 'white' }}>{cta2Text}</Link>
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {stats.map((stat, i) => (<div key={i} className="text-center rounded-xl p-3" style={{ background: isLight ? 'white' : 'rgba(255,255,255,0.05)', border: `1px solid ${isLight ? 'rgba(255,107,0,0.2)' : 'rgba(255,255,255,0.1)'}` }}><div className="font-display font-bold text-2xl text-primary">{stat.value}</div><div className="text-xs mt-0.5" style={{ color: isLight ? '#5C3D1E' : '#9CA3AF' }}>{stat.label}</div></div>))}
          </motion.div>
        </div>
      </div>
    </section>
