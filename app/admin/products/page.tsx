...
import { Plus, Edit2, Trash2, X, Save, Search, ToggleLeft, ToggleRight, Star } from 'lucide-react';
...
const EMPTY = { name:{en:'',hi:''}, slug:'', category:'rudraksha', description:{en:'',hi:''}, benefits:[], price:0, offerPrice:0, images:[''], stock:10, isFeatured:false, isNewLaunch:false, isActive:true, sku:'', seo:{title:'',description:''} };
...
export default function ProductsPage() {
  ...
  const load = () => { setLoading(true); productsAPI.getAdminAll().catch(()=>productsAPI.getAll()).then(r=>setProducts(r.data.data||[])).catch(()=>toast.error('Failed')).finally(()=>setLoading(false)); };
  useEffect(()=>{load();},[]);
  ...
  const addBenefit = () => { if(!newBenefit.trim())return; setModal((p:any)=>({...p,benefits:[...(p.benefits||[]),newBenefit.trim()]})); setNewBenefit(''); };

  const filtered = products.filter(p=>{
    const q=search.toLowerCase();
    const matchQ=!q||p.name.en.toLowerCase().includes(q)||p.category.includes(q);
    return matchQ&&(catFilter==='all'||p.category===catFilter);
  });

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="font-display text-2xl font-bold text-gray-800">Products</h1><p className="text-gray-500 text-sm mt-1">{products.length} products</p></div>
      ...
      <div className="flex gap-3">
        <div className="relative flex-1"><Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search products…" className="w-full pl-9 pr-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-primary"/></div>
        <select value={catFilter} onChange={e=>setCatFilter(e.target.value)} className="px-3 py-2.5 border border-gray-200 rounded-xl text-sm bg-white focus:outline-none focus:border-primary"><option value="all">All Categories</option>{CATS.map(c=><option key={c} value={c} className="capitalize">{c}</option>)}</select>
      </div>
        ...
        <div className="grid gap-3">
          {filtered.map(p=>(
            <div key={p._id} className="bg-white rounded-2xl border border-orange-100 p-4 shadow-sm flex items-center gap-4">
              <div className="w-12 h-12 rounded-xl overflow-hidden flex-shrink-0 bg-orange-50">{p.images?.[0]?<img src={p.images[0]} alt={p.name.en} className="w-full h-full object-cover"/>:<div className="w-full h-full flex items-center justify-center text-xl">🕉️</div>}</div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap"><h3 className="font-semibold text-gray-800 truncate">{p.name.en}</h3>{p.isFeatured&&<span className="text-xs px-2 py-0.5 rounded-full bg-yellow-100 text-yellow-700 font-semibold flex items-center gap-1"><Star size={9}/>Featured</span>}{p.isNewLaunch&&<span className="text-xs px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 font-semibold">New</span>}{!p.isActive&&<span className="text-xs px-2 py-0.5 rounded-full bg-gray-100 text-gray-500 font-semibold">Inactive</span>}</div>
                <p className="text-xs text-gray-400 mt-0.5 capitalize">{p.category} · {formatPrice(p.offerPrice)} <span className="line-through text-gray-300">{formatPrice(p.price)}</span> · Stock: {p.stock}</p>
              </div>
          ...
          {filtered.length===0&&<div className="text-center py-20 text-gray-400">No products found</div>}
        </div>
      )}
      ...
      {modal&&(
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/50 backdrop-blur-sm p-4 overflow-y-auto" onClick={e=>{if(e.target===e.currentTarget)closeModal();}}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl my-4">
            <div className="flex items-center justify-between p-5 border-b border-orange-100"><h2 className="font-display font-bold text-lg text-gray-800">{modal._id?'Edit Product':'New Product'}</h2><button onClick={closeModal} className="p-2 hover:bg-gray-100 rounded-lg"><X size={18}/></button></div>
            <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
              ...
              <div className="flex items-center gap-4">
                {[['isActive','Active'],['isFeatured','Featured'],['isNewLaunch','New Launch']].map(([f,l])=><label key={f} className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={!!(modal as any)[f]} onChange={e=>setModal((p:any)=>({...p,[f]:e.target.checked}))} className="rounded"/><span className="text-sm">{l}</span></label>)}
              </div>
