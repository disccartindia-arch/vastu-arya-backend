...
import { Plus, Edit2, Trash2, X, Save, Heart, MessageCircle, Search, Rss } from 'lucide-react';

const CATS = ['vastu-tip', 'remedy', 'transformation', 'astrology', 'numerology', 'motivation'];
const EMPTY = { caption: '', category: CATS[0], media: [{ url: '', type: 'image' }], isActive: true, author: 'Dr. PPS Tomar' };
...
export default function SocialPostsPage() {
  ...
  const openNew = () => setModal({ ...EMPTY, media: [{ url: '', type: 'image' }] });
  const openEdit = (p: any) => setModal({ ...p, media: p.media?.length ? p.media : [{ url: '', type: 'image' }] });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="font-display text-2xl font-bold text-gray-800">Vastu Feed</h1><p className="text-gray-500 text-sm mt-1">{posts.length} posts published</p></div>
      ...
      {loading ? (
        <div className="text-center py-20 text-gray-400"><div className="text-3xl animate-spin mb-2">🕉️</div>Loading posts…</div>
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(p => (
            <div key={p._id} className="bg-white rounded-2xl border border-orange-100 shadow-sm overflow-hidden group">
              {p.media?.[0]?.url ? (
                <div className="aspect-square overflow-hidden bg-orange-50 relative">
                  <img src={p.media[0].url} alt="" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"/>
                </div>
              ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/60 backdrop-blur-sm p-3 sm:p-6 overflow-y-auto" onClick={e => { if (e.target === e.currentTarget) setModal(null); }}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg my-4">
            ...
            <div className="p-5 space-y-5">
              ...
              <MobileImageUpload
                value={modal.media?.[0]?.url || ''}
                onChange={(url: string) => setModal((p: any) => ({ ...p, media: [{ url, type: 'image' }] }))}
                label="Post Photo"
                height="h-52"
              />
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1.5">Caption *</label>
                <textarea value={modal.caption || ''} onChange={e => setModal((p: any) => ({ ...p, caption: e.target.value }))} rows={5} placeholder="Share your Vastu tip, remedy or inspiration…" className="w-full px-3 py-2.5 border border-orange-200 rounded-xl text-sm resize-none focus:outline-none focus:border-primary"/>
              </div>
              ...
              <label className="flex items-center gap-3 cursor-pointer p-3 bg-green-50 rounded-xl border border-green-200">
                ...
                <div><p className="text-sm font-medium text-green-800">Visible on Vastu Feed</p><p className="text-xs text-green-600">Users will see this post on the feed</p></div>
              </label>
            </div>
