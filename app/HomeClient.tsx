...
export default function HomePage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      const allProducts: Product[] = pRes.data.data || [];
      setProducts(allProducts.filter(p => hasValidImage((p.images || [])[0])));
      ...
      if (hs) {
        setHeroSettings({
          ...
          stats:              hs.stats,
          // CHANGED: 100+ → 25+
          servicesButtonText: hs.servicesButtonText?.replace('100+', '25+') || 'View All 25+ Services',
        });
      }
  ...
  return (
    <>
      ...
      <main>
        ...
        <HomepageSlider />
        {/* CHANGED: 100+ → 25+ in button text fallback */}
        <FeaturedServices services={services} onBookAppointment={() => setShowAppointmentPopup(true)} servicesButtonText={heroSettings?.servicesButtonText || 'View All 25+ Services'} />
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 text-center text-white">
              {(heroSettings?.stats?.length ? heroSettings.stats.sort((a, b) => a.order - b.order) : tArr('home.hero.stats')).map((s: any, i: number) => (
                <motion.div key={i} initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}>
                  {/* CHANGED: replace any 100+ with 25+ for Services stat */}
                  <div className="font-display font-bold text-4xl sm:text-5xl mb-1">
                    {s.label === 'Services' || s.label === 'Services Offered' ? '25+' : s.value}
                  </div>
        ...
        {/* Featured Products — only with valid images */}
        {products.length > 0 && (
              ...
              {WHY_CARDS.map((card, i) => {
                ...
                return (
                  <motion.div key={i} {...fadeUp} transition={{ delay: i * 0.1 }} className="p-6 rounded-2xl border border-orange-100 hover:shadow-orange transition-all premium-card">
                    <div className="w-11 h-11 rounded-xl flex items-center justify-center mb-4" style={{ background: 'linear-gradient(135deg, rgba(212,160,23,0.12), rgba(255,107,0,0.08))' }}>
