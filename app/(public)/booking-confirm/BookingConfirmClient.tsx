    ...
    <div className="min-h-screen" style={{ background: 'linear-gradient(135deg, #FFFDF7 0%, #FFF8EE 50%, #FFFAF2 100%)' }}>
      <div className="max-w-xl mx-auto px-4 py-10 sm:py-16">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white rounded-2xl p-5 shadow-sm mb-5 text-center" style={{ border: '1px solid rgba(212,160,23,0.25)' }}>
          ...
          <div className="flex items-center justify-center gap-3"><span className="font-mono font-bold text-2xl" style={{ color: '#FF6B00' }}>{bookingRef}</span><button onClick={copyRef} className="p-2 hover:bg-orange-50 rounded-lg transition-colors"><Copy size={15} className={copied ? 'text-green-500' : 'text-gray-400'} /></button></div>
        </motion.div>
...
export default function BookingConfirmPage() {
  return (<><Navbar /><Suspense fallback={<div className="min-h-screen flex items-center justify-center"><div className="text-5xl animate-spin">🕉️</div></div>}><BookingContent /></Suspense><Footer /><WhatsAppButton /></>);
}
